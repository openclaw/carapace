...
const widgets = {
  select: () => {
    const options = [
      { label: "QuickStart (recommended)", hint: "Recommended local setup. Change details later with openclaw configure." },
      { label: "Advanced", hint: "Choose every gateway, model, and channel setting." },
    ...
    return {
      frame() {
        ...
        const rows = options.map((option, i) => {
          const hint = i === index ? ` ${INK.muted}(${option.hint})${INK.reset}` : "";
  ...
  multiselect: () => {
    const options = [
      { label: "session-memory", hint: "Save session context on /new or /reset" },
      { label: "command-logger", hint: "Log command events" },
      { label: "bootstrap-extra-files", hint: "Inject additional workspace files" },
    ];
    let index = 0;
    let filter = "";
    let done = false;
