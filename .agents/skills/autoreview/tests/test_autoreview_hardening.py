...
class AutoreviewHardeningTests(unittest.TestCase):
    ...
    def test_local_bundle_blocks_secret_in_staged_diff(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            path.write_text("token=" + "A" * 24 + "\n", encoding="utf-8")
            git(repo, "add", "tracked.txt")

            with self.assertRaisesRegex(SystemExit, "staged diff"):
                self.helper["local_bundle"](repo)

    def test_local_bundle_blocks_secret_in_unstaged_diff(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            path = repo / "tracked.txt"
            path.write_text("safe\n", encoding="utf-8")
            ...
            with self.assertRaisesRegex(SystemExit, "unstaged diff"):
                self.helper["local_bundle"](repo)

    def test_branch_and_commit_bundles_block_secret_diff(self) -> None:
        with tempfile.TemporaryDirectory() as tempdir:
            repo = init_repo(Path(tempdir))
            path = repo / "tracked.txt"
            path.write_text("safe\n", encoding="utf-8")
            ...
            with self.assertRaisesRegex(SystemExit, "branch diff"):
                self.helper["branch_bundle"](repo, base)
            with self.assertRaisesRegex(SystemExit, "commit diff"):
